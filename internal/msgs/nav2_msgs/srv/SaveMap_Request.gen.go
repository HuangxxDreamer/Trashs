...
func init() {
	typemap.RegisterMessage("nav2_msgs/SaveMap_Request", SaveMap_RequestTypeSupport)
	typemap.RegisterMessage("nav2_msgs/srv/SaveMap_Request", SaveMap_RequestTypeSupport)
}

type SaveMap_Request struct {
	MapTopic string `yaml:"map_topic"`// URL of map resourceCan be an absolute path to a file: file:///path/to/maps/floor1.yamlOr, relative to a ROS package: package://my_ros_package/maps/floor2.yaml
	MapUrl string `yaml:"map_url"`
	ImageFormat string `yaml:"image_format"`// Constants for image_format. Supported formats: pgm, png, bmp
	MapMode string `yaml:"map_mode"`// Map modes: trinary, scale or raw
	FreeThresh float32 `yaml:"free_thresh"`// Thresholds. Values in range of [0.0 .. 1.0]
...
func (t *SaveMap_Request) Clone() *SaveMap_Request {
	c := &SaveMap_Request{}
	c.MapTopic = t.MapTopic
	c.MapUrl = t.MapUrl
	c.ImageFormat = t.ImageFormat
	c.MapMode = t.MapMode
	c.FreeThresh = t.FreeThresh
	c.OccupiedThresh = t.OccupiedThresh
	return c
}
...
func (t _SaveMap_RequestTypeSupport) PrepareMemory() unsafe.Pointer { //returns *C.nav2_msgs__srv__SaveMap_Request
	return (unsafe.Pointer)(C.nav2_msgs__srv__SaveMap_Request__create())
}

func (t _SaveMap_RequestTypeSupport) ReleaseMemory(pointer_to_free unsafe.Pointer) {
	C.nav2_msgs__srv__SaveMap_Request__destroy((*C.nav2_msgs__srv__SaveMap_Request)(pointer_to_free))
}

func (t _SaveMap_RequestTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	m := msg.(*SaveMap_Request)
	mem := (*C.nav2_msgs__srv__SaveMap_Request)(dst)
	primitives.StringAsCStruct(unsafe.Pointer(&mem.map_topic), m.MapTopic)
	primitives.StringAsCStruct(unsafe.Pointer(&mem.map_url), m.MapUrl)
	primitives.StringAsCStruct(unsafe.Pointer(&mem.image_format), m.ImageFormat)
	primitives.StringAsCStruct(unsafe.Pointer(&mem.map_mode), m.MapMode)
	mem.free_thresh = C.float(m.FreeThresh)
	mem.occupied_thresh = C.float(m.OccupiedThresh)
}

func (t _SaveMap_RequestTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	m := msg.(*SaveMap_Request)
	mem := (*C.nav2_msgs__srv__SaveMap_Request)(ros2_message_buffer)
	primitives.StringAsGoStruct(&m.MapTopic, unsafe.Pointer(&mem.map_topic))
	primitives.StringAsGoStruct(&m.MapUrl, unsafe.Pointer(&mem.map_url))
	primitives.StringAsGoStruct(&m.ImageFormat, unsafe.Pointer(&mem.image_format))
	primitives.StringAsGoStruct(&m.MapMode, unsafe.Pointer(&mem.map_mode))
	m.FreeThresh = float32(mem.free_thresh)
	m.OccupiedThresh = float32(mem.occupied_thresh)
}

func (t _SaveMap_RequestTypeSupport) TypeSupport() unsafe.Pointer {
	return unsafe.Pointer(C.rosidl_typesupport_c__get_message_type_support_handle__nav2_msgs__srv__SaveMap_Request())
}

type CSaveMap_Request = C.nav2_msgs__srv__SaveMap_Request
type CSaveMap_Request__Sequence = C.nav2_msgs__srv__SaveMap_Request__Sequence
...
func SaveMap_Request__Sequence_to_C(cSlice *CSaveMap_Request__Sequence, goSlice []SaveMap_Request) {
	...
	cSlice.data = (*C.nav2_msgs__srv__SaveMap_Request)(C.malloc(C.sizeof_struct_nav2_msgs__srv__SaveMap_Request * C.size_t(len(goSlice))))
