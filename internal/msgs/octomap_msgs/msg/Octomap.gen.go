...
func init() {
	...
	typemap.RegisterMessage("octomap_msgs/msg/Octomap", OctomapTypeSupport)
}
...
func (t *Octomap) GetTypeSupport() types.MessageTypeSupport {
	return OctomapTypeSupport
}

// OctomapPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type OctomapPublisher struct {
	*rclgo.Publisher
}

// NewOctomapPublisher creates and returns a new publisher for the
// Octomap
func NewOctomapPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*OctomapPublisher, error) {
	pub, err := node.NewPublisher(topic_name, OctomapTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &OctomapPublisher{pub}, nil
}

func (p *OctomapPublisher) Publish(msg *Octomap) error {
	return p.Publisher.Publish(msg)
}

// OctomapSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type OctomapSubscription struct {
	*rclgo.Subscription
}

// OctomapSubscriptionCallback type is used to provide a subscription
// handler function for a OctomapSubscription.
type OctomapSubscriptionCallback func(msg *Octomap, info *rclgo.MessageInfo, err error)

// NewOctomapSubscription creates and returns a new subscription for the
// Octomap
func NewOctomapSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback OctomapSubscriptionCallback) (*OctomapSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Octomap
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, OctomapTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &OctomapSubscription{sub}, nil
}

func (s *OctomapSubscription) TakeMessage(out *Octomap) (*rclgo.MessageInfo, error) {
...
func Octomap__Sequence_to_Go(goSlice *[]Octomap, cSlice COctomap__Sequence) {
	...
	*goSlice = make([]Octomap, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		OctomapTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func Octomap__Sequence_to_C(cSlice *COctomap__Sequence, goSlice []Octomap) {
	...
	cSlice.data = (*C.octomap_msgs__msg__Octomap)(C.malloc(C.sizeof_struct_octomap_msgs__msg__Octomap * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		OctomapTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
