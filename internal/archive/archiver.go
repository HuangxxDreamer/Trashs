...
func (a *Archiver) StartArchive(latestMapBase64 []byte) {
	a.wg.Add(1)
	go func() {
		defer a.wg.Done()
		...
		if err := os.WriteFile(mapPath, latestMapBase64, 0644); err != nil {
		...
		if exportCmdStr != "" {
			...
			// 简单的命令分割（工业级实现可能需要更复杂的解析）
			parts := strings.Fields(exportCmdStr)
